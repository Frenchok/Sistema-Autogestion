...
let userController = {
    ...
    list: async function (req, res) {
        ...
        try {
            const results = await userService.getByRol(page);
            res.render('./allUser', { user: results.user, previous: results.previous, next: results.next, page })
        } catch (error) {
            console.log(error);
    ...
    getAllUsers: async (req, res) => {
        try {
          let users = await userService.getByRol({ where: { rol: 'comun' } });
          res.json(users);
        } catch (error) {
